...
### ✅ 完了（2026-04-16 ci.yml notify/dispatch_deploy 移行）

| ワークフロー | 移行前 | 移行後 | 変更内容 |
|---|---|---|---|
| `ci.yml` (notify ジョブ) | `ubuntu-latest` | `[self-hosted, sakura-vps]` | `jq` → `python3`、Secrets を env 変数経由に変更 |
| `ci.yml` (dispatch_deploy ジョブ) | `ubuntu-latest` | `[self-hosted, sakura-vps]` | 変更なし（actions/github-script） |

### ✅ 完了（2026-04-19 copilot-setup-steps / pr_guardrails を self-hosted 化）

| ワークフロー | 移行前 | 移行後 | 変更内容 |
|---|---|---|---|
| `copilot-setup-steps.yml` | `ubuntu-latest` + Docker services + `ruby/setup-ruby` + `actions/setup-node` | `[self-hosted, sakura-vps]` | services 削除、rbenv 方式、VPS の PostgreSQL/Redis/Node.js を使用 |
| `pr_guardrails.yml` | `ubuntu-latest` | `[self-hosted, sakura-vps]` | Python3 スクリプトのみなので変更最小限 |

> **目的**: GitHub ホステッドランナーの Linux minutes 消費を 0 に削減（Copilot セッション毎に 10〜15分 × 日5〜20回 = 50〜300分/日 → 0分/日）
...
### 🔧 self-hosted runner オフライン監視（2026-04-15、2026-04-16 改善）

**追加**: `bin/check_runner_health` スクリプトを作成。

- runner サービスが停止していたら**自動リスタートを試行**（`sudo systemctl restart`）
- リスタート成功 → ログ出力のみ（通知なし、自動復旧）
- リスタート失敗 → Slack（`SLACK_WEBHOOK_URL_ERROR`）に通知
  - **1時間のクールダウン**付き（同じ障害で繰り返し通知しない）
  - クールダウン状態ファイル: `/tmp/runner_health_last_notify`
  - runner 復旧時にクールダウンをリセット（次の障害時に即通知可能に）
...
| 観点 | Picro通知 |
|---|---|
| 独立性 | ✅ 完全独立（AI SNS と共有データなし） |
| 実装規模 | 小（3ファイル + 1モデル + 1ジョブ） |
| スケーリング要件 | なし（1日数回の定期実行） |
| デプロイ頻度 | 低い |
| 外部依存 | LINE API, Picro（スクレイピング） |
| DB共有 | Solid Queue, picro_messages テーブル |
| 切り出しのコスト | 中（新リポ作成 + 別サーバーor同VPS別プロセス） |
...
理由:
1. **運用コスト > メリット**: 実行頻度が低く（Picro: 毎日数回）、スケーリングの必要がない
...
**切り出しを検討するタイミング（将来）**:
- Picro が主要機能として成長し、デプロイ頻度が AI SNS と乖離したとき
...
- Picro を切り出した場合 → 切り出し先の軽量サービスに Cloudflare Workers/Render 等を使うことは可能
...
| 2026-04-12 | モノリスを維持（Picro の切り出しは行わない） | このドキュメント |
...
| 2026-04-16 | `bin/check_runner_health` 改善: 自動リスタート + 1時間クールダウン | このPR |
| 2026-04-16 | `ci.yml` notify/dispatch_deploy ジョブを self-hosted に移行（Linux minutes 削減） | このPR |
| 2026-04-19 | `copilot-setup-steps.yml` を self-hosted に移行（Docker services 廃止、rbenv/VPS PostgreSQL 使用） | このPR |
| 2026-04-19 | `pr_guardrails.yml` を self-hosted に移行 | このPR |
| 2026-04-27 | 全13ワークフロー self-hosted 化完了・動作確認済み。プロジェクト完了 | 確認済み |
...
- [x] `bin/check_runner_health` 改善: 自動リスタート + 1時間クールダウン追加（2026-04-16）
- [x] `ci.yml` notify/dispatch_deploy を self-hosted に移行: jq→python3, Linux minutes 削減（2026-04-16）
- [x] `db_snapshot.yml` を手動 dispatch して修正後の動作を確認（完了）
- [x] `copilot-setup-steps.yml` を self-hosted に移行: Docker services 廃止、rbenv/VPS PostgreSQL/Redis 使用（2026-04-19）
- [x] `pr_guardrails.yml` を self-hosted に移行（2026-04-19）
- [x] 一括移行後の各ワークフロー動作確認（全13ワークフロー全て `[self-hosted, sakura-vps]` で稼働確認済み、2026-04-27）
...
| `.github/workflows/ci.yml` | CI（self-hosted runner、`dispatch_deploy` ジョブ含む） |
| `.github/workflows/copilot-setup-steps.yml` | Copilot環境構築（self-hosted runner、rbenv + VPS PostgreSQL/Redis） |
| `.github/workflows/pr_guardrails.yml` | PR本文検証（self-hosted runner、Python3スクリプト） |
