...
| ワークフロー | 最終確認日 | 結果 |
|---|---|---|
| `deploy.yml` | 2026-04-14 | ✅ 安定稼働中（self-hosted runner に再実装済み、SSH key 不要に） |
| `db_snapshot.yml` | 2026-04-13 | ⚠️ 最新実行が失敗（下記参照）→ 修正済み（このPR） |
| `weekly_pdca.yml` | 2026-04-14 | ⚠️ WIP dispatch の if 条件バグ修正済み（下記参照）、次回実行で確認 |
...
**問題4（調査中）**: スケジュール実行が 2026-04-13 05:04 以降停止している。コード修正後の schedule 実行がまだ行われていない。GitHub が連続失敗後にスケジュールを一時停止した可能性あり。Actions タブから手動で Re-run するか、`workflow_dispatch` で動作確認が必要。
...
| 2026-04-14 | `weekly_pdca.yml` WIP dispatch の if 条件バグ修正 | このPR |
...
- [ ] `deploy.yml` を次回 CI 通過後に動作確認（self-hosted 再実装後の初回テスト）
- [ ] `db_snapshot.yml` を手動 dispatch して runner 動作を確認（再登録後 + 修正後の初回テスト）
- [ ] `weekly_pdca.yml` を手動 dispatch または Re-run でスケジュール実行の再開を確認
