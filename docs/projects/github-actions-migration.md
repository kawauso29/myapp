...
### ⚠️ 要対応（`db_snapshot.yml` の権限エラー）

**状況**: 初回実行（2026-04-12）が `git push` の 403 エラーで失敗。

**原因**: ワークフロー内の `permissions` が `contents: read` のままで `db-snapshots` ブランチへの書き込みが拒否された。

**修正方法**（未実施）:
```yaml
permissions:
  contents: write  # ← read から write に変更
```
...
| `db_snapshot.yml` | 2026-04-12 | ❌ git push 403（権限エラー。上記参照） |
...
| 2026-04-12 | さくらVPS を維持（Cloudflare 等への移行は行わない） | このドキュメント |

---

## 6. 今後のアクション（TODO）

- [ ] `db_snapshot.yml` の `permissions: contents: write` 修正（担当: 次の AI セッション）
- [ ] self-hosted runner のオフライン監視（GitHub Settings → Actions → Runners で定期確認）
- [ ] weekly_pdca.yml の初回実行確認（2026-04-13 月曜 9:00 JST）
