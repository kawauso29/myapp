...
| ワークフロー | 最終確認日 | 結果 |
|---|---|---|
| `deploy.yml` | 2026-04-14 | ✅ 安定稼働中（直近5件中3件 success、2件は CI 失敗による skipped） |
| `db_snapshot.yml` | 2026-04-13 | ⚠️ 最新実行が失敗（下記参照）→ 修正済み（このPR） |
| `weekly_pdca.yml` | 2026-04-13 | ⚠️ スケジュール実行 8 件中全件 failure（下記参照）→ コード修正は反映済み、次回実行で確認 |
...
**問題3（調査中）**: スケジュール実行が 2026-04-13 05:04 以降停止している。コード修正後の schedule 実行がまだ行われていない。GitHub が連続失敗後にスケジュールを一時停止した可能性あり。Actions タブから手動で Re-run するか、`workflow_dispatch` で動作確認が必要。
