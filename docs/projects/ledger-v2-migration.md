...
**2026-05-03: Ticket 24 完了。`LedgerV2::CollectCustomerFeedback` で v1 CustomerFeedbackLedger を readonly 観測。DailyRunner に customer_feedback_new_count / customer_feedback_escalated_count を追加。自動Ticket生成なし（人間承認必須）。**
...
- Admin UI `/admin/ledger_v2` で Run / Ticket / Artifact / HealthSnapshot / Monthly Runner を観察できる
- DailyRunner の観測 KPI: AI-SNS 3指標 + CustomerFeedback 2指標 + error / ci_success_rate / open_ticket / artifact_pending（計9指標）

次のステップ:
1. **Ticket 25（KnowledgeLedger または ExperimentLedger など Phase G-4 #2）**を設計して進める
2. `customer_feedback_escalated_count` の異常 Ticket が生成されたら Dashboard で確認し、必要に応じて閾値を調整する
