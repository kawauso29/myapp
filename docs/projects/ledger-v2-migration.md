...
**2026-05-04: Ticket 26 完了。`LedgerV2::CollectExperimentMetrics` で v1 ExperimentLedger を readonly 観測。DailyRunner に experiment_active_count / experiment_expired_count を追加。自動Ticket生成なし（人間承認必須）。**
...
次のステップ:
1. **Ticket 27（Quarterly/Annual など Phase G-4 #4、または ExperimentLedger 以降の次候補）**を設計して進める
2. `experiment_expired_count` の異常 Ticket が生成されたら Dashboard で確認し、必要に応じて閾値を調整する
