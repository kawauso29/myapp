...
**Ticket 22 完了後は Ticket 23（Monthly dry_run 7圧縮日観察）へ進む。**
**2026-05-03: Dashboard の v2 卒業判定が ALL PASS（HealthSnapshot 11 件）になったため、Layer C 接続の設計レビューを開始可能。**
**2026-05-03: Ticket 23 実装完了。`monthly_runner` フラグ有効化・MonthlyRunnerJob を dry_run: true デフォルトに修正・観察 spec（14 examples, 0 failures）作成。本番 Dashboard で 3.5 時間以上 HealthSnapshot が蓄積され GraduationCheck ALL PASS を維持するまで `[x]` を付けない。**
...
次のステップ:
1. Ticket 22 マージ後、**Ticket 23（Monthly dry_run 7圧縮日観察）**に進む
2. `ledger_v2_monthly_runner` フラグを `true` に設定し、Dashboard で MonthlyRunner の実行を観察する
3. Monthly 起因でノイズ率 / 採用率 / 失敗率が悪化していないことを HealthSnapshot で確認する
