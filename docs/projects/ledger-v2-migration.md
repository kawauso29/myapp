...
**2026-05-04: Ticket 27 完了。`LedgerV2::QuarterlyRunner` 骨組みを実装。Monthly Artifact を四半期 draft に集約する BuildQuarterlyArtifact + QuarterlyRunnerJob（manual only、recurring なし）。FeatureFlag `quarterly_runner` は false のまま維持。**

現在の状態:
- `config/initializers/ledger_v2.rb`: `quarterly_runner: false`（manual dispatch のみ実行可能）
- QuarterlyRunnerJob は recurring.yml 未登録（人間が手動 dispatch のみ）
- Admin UI `/admin/ledger_v2` Artifacts で `quarterly_review` フィルター対応済み
...
次のステップ:
1. **Ticket 28（Phase G-4 #5 Auto PR draft / または次の Layer C 候補）**を検討する
2. QuarterlyRunner を試したい場合は `LedgerV2::QuarterlyRunnerJob.perform_now(triggered_by: "admin")` で手動 dispatch できる（dry_run: true のみ）
