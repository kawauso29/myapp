...
| ④ Ledger（台帳）通知 | Ledger操作（チケット作成、保留、延滞マーク、改善提案）の運用ログ | `SLACK_WEBHOOK_URL_LEDGER` |
...
| Rails `Ledgers::SlackNotifier` | Ledger操作結果（tickets_created, held_items, overdue_marked, improvements） | ④ |

## 運用ルール

- RailsアプリのWebhookは `deploy.yml` の `Sync Slack env vars to VPS` で同期する（`SLACK_WEBHOOK_URL_ERROR` / `SLACK_WEBHOOK_URL_JOBS` / `SLACK_WEBHOOK_URL_LEDGER`）
- `channel: :jobs` の通知は `SLACK_WEBHOOK_URL_JOBS` 未設定時に **error へフォールバックしない**
- `Ledgers::SlackNotifier` は `SLACK_WEBHOOK_URL_LEDGER` 未設定時に `SLACK_WEBHOOK_URL`（旧来）へフォールバックし、それも未設定の場合は通知をスキップする
