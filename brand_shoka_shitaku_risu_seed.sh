#!/usr/bin/env bash
#
# brand_shoka_shitaku_risu_seed.sh
# ------------------------------------------------------------------
# LINEスタンプ工房 — ブランド「初夏したくリス」(shoka_shitaku_risu)
# Brand + 初回 Pack(8 stamps) を 1 ファイルに同梱して投入する。
# 由来リサーチ: weekly_trends_2026_w24 の brand_idea C。
#
# === なぜ ローカルで ruby を叩かないか ===
#   この環境(原田さんの Win/Git-Bash)には ruby が無い。
#   slug 検証は CI seed-check、DB apply は push 後に本番VPS が実行する。
#   → このスクリプトは「seed を書いて push するだけ」。ruby/rails は叩かない。
#
# 冪等。再実行しても seed 内容が同じなら差分なしで push をスキップ。
# 使い方: リポジトリのルート(myapp)で  bash brand_shoka_shitaku_risu_seed.sh
# ------------------------------------------------------------------
set -euo pipefail

if [ ! -f Gemfile ] || [ ! -f config/application.rb ]; then
  echo "ERROR: リポジトリのルートで実行してください (myapp で bash brand_shoka_shitaku_risu_seed.sh)" >&2
  exit 1
fi

PENDING_DIR="db/seeds/linestamp/imports/pending"
SEED_ID="2026-06-03-001502_brand_shoka-shitaku-risu"
...
if git diff --cached --quiet; then
  ...
  git commit -m "feat(linestamp): ブランド「初夏したくリス」を追加

weekly_trends_2026_w24 brand_idea C 由来。体より大きいふさふさ尾の
shoka_shitaku_risu を Brand + 初回 Pack(8 stamps) 同梱で作成。
朝の挨拶/身支度/軽い外出・食の誘いを core CT に据える。緑被り回避で
primary_color は初夏のアプリコット橙。CI 通過後、本番 apply で反映。"
...
echo
echo "============================================================"
echo " 完了: ブランド「初夏したくリス」(shoka_shitaku_risu)"
