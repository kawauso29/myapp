#!/usr/bin/env bash
#
# brand_ajisai_katatsumuri_seed.sh
# ------------------------------------------------------------------
# LINEスタンプ工房 — ブランド「あじさいカタツムリの“ゆっくりでいい”」(ajisai_katatsumuri)
# Brand + 初回 Pack(8 stamps) を 1 ファイルに同梱して投入する。
# 由来リサーチ: weekly_trends_2026_w24 の brand_idea B。
#
...
# 使い方: リポジトリのルート(myapp)で  bash brand_ajisai_katatsumuri_seed.sh
# ------------------------------------------------------------------
set -euo pipefail

if [ ! -f Gemfile ] || [ ! -f config/application.rb ]; then
  echo "ERROR: リポジトリのルートで実行してください (myapp で bash brand_ajisai_katatsumuri_seed.sh)" >&2
  exit 1
fi

PENDING_DIR="db/seeds/linestamp/imports/pending"
SEED_ID="2026-06-03-001501_brand_ajisai-katatsumuri"
...
if git diff --cached --quiet; then
  ...
  git commit -m "feat(linestamp): ブランド「あじさいカタツムリの“ゆっくりでいい”」を追加

weekly_trends_2026_w24 brand_idea B 由来。横長の渦巻き殻を背負った
梅雨モチーフ ajisai_katatsumuri を Brand + 初回 Pack(8 stamps)
同梱で作成。離席/即レス/同意/労りを core CT に据える。
...
echo
echo "============================================================"
echo " 完了: ブランド「あじさいカタツムリの“ゆっくりでいい”」(ajisai_katatsumuri)"
