...
会話（セッション）の途中でPRが自動マージされてセッションが終了しないよう、**自動作成されたPRには `session-hold` ラベルが自動付与される**。

### 仕組み

```
push → create_pr.yml でPR作成（session-hold ラベルを自動付与）
    ↓
CI 通過 → auto_merge.yml が session-hold を検出 → マージをスキップ
    ↓
会話完了後: GitHubのPRページで session-hold ラベルを外す（2クリック）
    ↓
auto_merge.yml の unlabeled トリガー発火 → CI通過済みならそのまま即マージ → デプロイ
```
...
- `GITHUB_TOKEN` で作成したコメント/Issueは GitHub Apps（Copilot coding agent）の Webhook をトリガーしない（GitHub のループ防止仕様）。ただし `DEPLOY_TOKEN` の権限不足で `issues.createComment` が 403 になる場合がある → **正しい対処**: `ai_sns_plan.yml` の `@copilot` コメント投稿は `DEPLOY_TOKEN` で先に試行（`continue-on-error: true`）し、失敗時のみ `GITHUB_TOKEN` にフォールバックする。併せてワークフローに `permissions: issues: write` を付与する
