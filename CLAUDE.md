...
| プロジェクト | ドキュメント | 状態 |
|---|---|---|
| GitHub Actions 自前移行（self-hosted runner） | `docs/projects/github-actions-migration.md` | ✅ 完了（ci.yml テスト5ジョブ: scan_ruby/lint/job-check/route-check/test。copilot-setup-steps.yml/build_frontend以外は全て移行済み） |
| 運営 OS Phase 30〜41 実装 | `docs/projects/operating-spec-phase-30-plan.md` | ✅ ほぼ完了（Phase 30〜37 / 39 / 38b / 41b 完了 + 補強9 template_id + enforce モード機構 + Phase 40 LlmGateway augment hook + Phase 42 圧縮時間軸の cron / Runner 統合。本番での enforce ON タイミングのみ残件） |
