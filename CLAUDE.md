...
- **Copilot が `copilot/ai-sns-*` ではなく `copilot/copilotledger-*` ブランチで実装を完了したとき**: `copilot/ai-sns-*` ブランチの PR は変更ファイルが0件の空PRとして残り、`auto_merge` の空PRガードでマージされず永久に open になる。plan status（TicketLedger）も `Update AI SNS plan status to done` ステップが `ai-sns-{KEY}` パターンにマッチしないため done 更新されない。**対処**:①`auto_merge.yml` の空PRガード（`merge_on_ci_pass` / `undraft_ai_sns_prs` スケジュール）が空の ai-sns-* PRを自動クローズするように修正済み。②B3 plan status を本番DBで更新: `cd ~/myapp && RAILS_ENV=production bin/rails runner "t = TicketLedger.find_ai_sns_plan_by_item_key('B3'); t.skip_template_guard=true; t.skip_lane_capacity_guard=true; t.skip_pr_guardrail=true; t.skip_stop_guard=true; t.update!(status: :completed)"`
