# LedgerV2::DailyRunnerJob — DailyRunner を RunExecutor 経由で起動するジョブ。
#
# 責務:
# - RunExecutor.call(:daily_runner, ...) を呼ぶだけ
# - スケジュール実行（config/recurring.yml）と手動 dispatch に対応する
# - dry_run: true を渡せば副作用なしで実行できる
#
# 設計の正本: ledger_v2_detailed_design.txt §「LedgerV2::DailyRunnerJob」
module LedgerV2
  class DailyRunnerJob < ApplicationJob
    queue_as :default
