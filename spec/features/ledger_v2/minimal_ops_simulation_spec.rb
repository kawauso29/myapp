...
RSpec.describe "LedgerV2 MVP 最小完成条件 総点検 / Ticket 18: 7日間シミュレーション",
  ...
  describe "7日間シミュレーション（DailyRunner × 7 + WeeklyRunner × 1）" do
    ...
    # CollectAiSnsMetrics(3件) + CollectCustomerFeedback(2件) + CollectKnowledgeMetrics(2件) + snapshot_for × 4件(error/ci/open_ticket/artifact_pending) = 11件/日
    it "MVP 5: MetricSnapshot が保存される（11件/日 × 7日 = 77件）" do
      expect(LedgerV2::MetricSnapshot.count).to eq(77)
    end
  ...
  describe "MVP 12: dry_run ができる" do
    ...
    it "dry_run=true でも MetricSnapshot は保存される（観測ファクトは残す）" do
      expect {
        run_daily_on(0, dry_run: true)
      }.to change(LedgerV2::MetricSnapshot, :count).by(11)
    end
  end
