...
- `GITHUB_TOKEN` で作成したコメント/Issueは Copilot coding agent の Webhook をトリガーしない（GitHub のループ防止仕様）→ `@copilot` メンションを含むコメント/Issue作成時は `github-token: ${{ secrets.DEPLOY_TOKEN || secrets.GITHUB_TOKEN }}` で PAT を使う
