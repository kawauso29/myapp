...
- `GITHUB_TOKEN` で作成したコメント/Issueは Copilot coding agent の Webhook をトリガーしない（GitHub のループ防止仕様）。ただし `DEPLOY_TOKEN` の権限不足で 403 になる場合があるため、`ai_sns_plan.yml` の `@copilot` コメント投稿は **DEPLOY_TOKEN で先に試行（`continue-on-error: true`）し、失敗時のみ GITHUB_TOKEN にフォールバック**する。あわせて `permissions: issues: write` を必ず付与する
