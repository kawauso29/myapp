...
| Ledger V2 移行（v1 反省を踏まえた最小 Kernel 構築） | `docs/projects/ledger-v2-migration.md`（設計の正本は `ledger_v2_detailed_design.txt`） | 🔧 進行中（**v1 自動化 GitHub Actions 9 本を削除し v1 freeze 完了**: `ai_sns_plan` / `weekly_pdca` / `plan_review` / `auto_merge` / `ci_escalation` / `auto_fix` / `create_pr` / `post_deploy_cleanup` / `pr_guardrails`。残存 workflow は `ci` / `deploy` / `copilot-setup-steps` / `db_snapshot` の 4 本のみ。**v1 自動化機構を v2 構築に流用しない**・**1 PR 1 機能 1 テスト**・**機構安定まで自動マージ一時停止**（安定後は自動マージ・自動デプロイへ移行）を厳守） |
...
- Sprint H で実装した `linestamp-research.yml` / `linestamp-brand-planning.yml` / `linestamp-pack-planning.yml` / `linestamp-sync.yml` は **`runs-on: [self-hosted, sakura-vps]`** を使う。Issue 起票系3本は `GH_TOKEN=${{ secrets.DEPLOY_TOKEN }}` で実行し、**Issue作成 → `@copilot` コメント投稿 → `copilot-swe-agent[bot]` へ `agent_assignment` 付き assignee 追加** の順を厳守する。self-hosted には `jq` が無いので GitHub API payload は `python3` で JSON 生成する
...
- **定期実行を前提にした観測モデル / Runner を追加するときは、必ず ①`config/recurring.yml` 登録 + ②`config/initializers/required_job_classes.rb` と `lib/tasks/solid_queue.rake` の `REQUIRED_JOB_CLASSES` 追加 + ③`*_job_spec.rb` の3点セットを同 PR で完結させる**。コードと spec が緑でも、recurring 未登録なら本番で「静かにゼロ件」になる。実例: LedgerV2 Ticket 16 の `CalculateHealthSnapshot` は service / model / spec が完成していたが recurring 登録漏れで snapshot 行が蓄積されず、Ticket 18「7 日間運用テスト完了」の `[x]` チェックが事実上ウソになっていた（2026-05-01 発覚・ロールバック）。レビュー時は「このコードを定期で動かすのは誰か」を必ず確認する
