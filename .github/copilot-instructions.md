...
- Copilot coding agent を Issue から起動するには `@copilot` コメントだけでなく、issueの **assignees に `copilot` を追加** することが必要。`GithubIssueService.add_assignees(issue_number:, assignees: ['copilot'])` を `@copilot` コメント投稿の**前**に呼ぶ（`TicketIssueSync#post_copilot_comment` で実装済み）。assignee 追加なしのコメントメンションだけでは Copilot coding agent が反応しない
