...
- `deploy.yml` の `workflow_run` トリガーが self-hosted runner 移行後に発火しなくなった → `ci.yml` に `dispatch_deploy` ジョブを追加し、main の CI 成功後に `deploy.yml` を `workflow_dispatch` で直接起動する（auto_merge.yml と同じ方式）
